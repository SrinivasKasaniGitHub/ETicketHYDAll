...
public class DrunkResponse extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            if (GenerateDrunkDriveCase.isDuplicatePrint) {
                db.insertDuplicatePrintDetails(""
...
//		tv_remarks.setText("");
        ...
        try {
            if (GenerateDrunkDriveCase.isDuplicatePrint) {
                Tv_printresponse.setText(ServiceHelper.str_SameChlnDuplicatePrint);
                printdata = ServiceHelper.str_SameChlnDuplicatePrint;
            } else {
                if (ServiceHelper.final_reponse_split.length > 0 ) {
                    Tv_printresponse.setText(ServiceHelper.final_reponse_split[0].split("\\^")[0]);
                    printdata = ServiceHelper.final_reponse_split[0].split("\\^")[0];
                }else {
